...
Filen `TestData1000.txt` innehåller 1000 rader med namn (t.ex "Stop ACF") och nummer (t.ex 1432) vilka är separerade med `:`.   
Du kan använda följande kod för en metod för att läsa från fil och skapa en array med osorterade BusStop-objekt.  
Tänk på att anpassa metoden avseende access (private eller public) och om den ska tillhöra klassen eller objekten.   
Parametern `nrOf` motsvarar det antal du som ska läsas från filen

```csharp
BusStop[] ReadFromFile(string filename, int nrOf)
{
    if (nrOf > 1000)
        nrOf = 1000;
    ...
    {
        int index = stops[i].IndexOf(":");
        busStops[i] = new BusStop(stops[i].Substring(0, index), int.Parse(stops[i]!.Substring(index+1)));
    }
    return busStops;
}
