...
...
```
Du kan använda följande kod för att läsa från fil och skapa en array med osorterade BusStop-objekt, där de 8 sista siffrorna i stop_id blir nr och `_name` blir stop_name.

Tänk på att anpassa metoden som ska läsa från fil avseende access (private eller public) och om den ska tillhöra klassen eller objekten.   
Parametern `nrOf` motsvarar det antal som ska läsas från filen mne om dess värde är -1 läses hela filen.
