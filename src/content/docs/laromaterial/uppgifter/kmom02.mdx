...
```csharp
public static void Quicksort<T>(T[] A, int start, int end) where T : IComparable<T> {/*ska implementeras*/}
...
Notera resultateten i en tabell där det tydligt framgår för varje sorteringsalgoritm vilket underlag det varit på elementen som sorterats (slumpade, sorterade resp omvänt sorterade) och hur många jämförelser som genomförts.

Notera dina slutsatser och redogör för dessa genom att relatera till de resultat du fått. Utgå från de frågor som finns i texten med rubriken "Undersök algoritmernas effekivitet"
